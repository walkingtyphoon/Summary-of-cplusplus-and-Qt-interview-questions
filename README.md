...
2.  ### 动态多态
    ...
    其本质上是继承的前提下,通过在父类中声明`虚函数`,虚函数分为两种`普通虚函数`和`纯虚函数`的区别在于一个可以有函数体,另一个必须没有,且被替换为`=0;`如果是普通的`虚函数`,则重写与否都不影响正常使用,而对于`纯虚函数`的话必须重写,然后将父类指针或者引用指向子类对象来实现`动态多态`,其本质上是当我们在类中声明了一个`虚函数时`,编译器会在对象中添加一个指向虚函数表的指针也就是所谓的虚指针,该表存储了虚函数的地址,每个具有`虚函数`的类都有其自己的`虚函数表`,当通过父类指针或者引用调用`虚函数`时,实际执行的函数是由对象的`虚函数表`所决定的,就是通过`虚函数表`找到正确的`虚函数`地址进行调用,进而达到`动态多态`的目的,`动态多态的话是在运行时期`,而且`父类`的`析构函数`还必须设置为`虚析构`,因为这样可以避免子类开辟的空间没有释放
...
## 静态类型转换和动态类型转换的区别是什么?`提问次数0`

1.  ### 时间不同

    静态类型转换是在编译时期,而动态类型转换是在运行时期
2.  ### 是否存在安全检查不同

    静态类型转换不存在安全类型检测,而动态类型转换则存在类型安全检查
3.  ### 是否支持多态不同

    静态类型转换不适用于多态,而动态类型检查适用于多态
4.  ### 支持的类型不同

    静态类型转换适合于基本数据类型,以及用户的自定义类型等;而动态类型转换主要用于多态类型之间的转换，需要在类之间存在虚函数。

总体来说，静态转换更适用于开发者有明确掌控并确保安全性的情况，而动态转换更适用于处理多态类型的情况，能够在运行时检查类型信息。
